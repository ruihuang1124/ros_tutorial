...
int main(int argc, char *argv[])
{
    setlocale(LC_ALL,"");
    // 2.初始化 ROS 节点
    ros::init(argc,argv,"Server_1");
    // 3.创建 ROS 句柄
    ros::NodeHandle nh;
    // 4.创建 服务 对象
//    ros::ServiceServer server = nh.advertiseService("AddInts",doReq);
    ros::ServiceServer server1 = nh.advertiseService("/panda_left/execute_joint_position_srv_control",doReq1);
//    ros::ServiceServer server2 = nh.advertiseService("/panda_right/execute_joint_position_srv_control",doReq2);
//    ros::ServiceServer server3 = nh.advertiseService("/curi/torso/execute_joint_position_srv_control",doReq3);
    ROS_INFO("服务已经启动....");
    //     5.回调函数处理请求并产生响应
    //     6.由于请求有多个，需要调用 ros::spin()
    ros::spin();
